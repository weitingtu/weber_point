...
void VisibilityGraph::_dijkstra(const QVector<QVector<double> >& w, int source_size, int source, QVector<double>& d, QVector<int>& parent)
{
...
void VisibilityGraph::create( const QVector<QPointF>& sources, const QVector<QPolygonF>& obs, const QVector<QPointF>& targets )
{
    ...
    for(int i = sources.size(); i < points.size() - 1; ++i)
    {
        int idx1 = i;
        int idx2 = i+1;
    ...
    for(int i = 0; i < sources.size(); ++i)
    {
        QVector<double> d;
        QVector<int> parent;
        _dijkstra( w, sources.size(), i, d, parent);
